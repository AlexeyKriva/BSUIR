...
public class HashTable {
    ...
    public ArrayList<Cell> deleteValue(String key){
        ...
        for (int i = id; i < cells.size(); i++){
            ...
            if (key.equals(cells.get(i).key)){
                cells.set(i, new Cell("", DELETE_INDEX, "", true));
                flag = true;
            }
        }
    ...
    public ArrayList<Cell> getInfo(String key){
        ArrayList<Cell> info = new ArrayList<>();
        int id = getIndex(key);
        boolean flag = false;
        for (int i = id; i < cells.size(); i++){
            if (!cells.get(i).state){
    ...
    public void showTable(){
        for (int i = 0; i < this.cells.size(); i++)
            System.out.println(i +". "+this.cells.get(i));
    }
}
